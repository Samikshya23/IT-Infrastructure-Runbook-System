$(document).ready(function () {
    ...
    $("#CategoryId").off("change").on("change", function () {
        ...
        $.get("/FormConfiguration/Add", { categoryId: categoryId }, function (data) {
    ...
    $("#btnAddRoot").off("click").on("click", function () {
        if ($("#CategoryId").val() === "") {
            showToastMessage("error", "Please select category first.");
            return;
        }
    ...
    function saveData() {
        ...
        if (categoryId === "") {
            showToastMessage("error", "Please select category.");
            return;
        }
        ...
        $.ajax({
            url: "/FormConfiguration/SaveStructure",
            type: "POST",
            ...
            success: function (response) {
                if (response.success) {
                    window.location.href =
                        "/FormConfiguration?selectedCategoryId="
                        + categoryId
                        + "&successMessage="
            ...
            error: function (xhr) {
                if (xhr.status === 400) {
                    showToastMessage("error", "Security token missing or expired. Please reload the page and try again.");
                    return;
                }
